...
def single_reply(timestamp, channel_id, response_url=None):
    params = {
        # "token": os.environ["SLACK_USER_TOKEN"],
        "channel": channel_id,
        "ts": timestamp,
...
def parse_channel_history(msgs, users, channel_id, check_thread=False):
    if "messages" in msgs:
        msgs = msgs["messages"]
    ...
    for msg in messages:
        ...
        if "reply_count" in msg:
            reply_messages = single_reply(msg["ts"], channel_id)
            
            for index, msg in enumerate(reply_messages):
                if "user" in msg:
                    usr = {
                        "name": name_from_uid(msg["user"], users),
                        "real_name": name_from_uid(msg["user"], users, real=True),
                ...
                if index == 0:
                    ...
                    # reply_timestamps = [x["ts"] for x in msg if "reply_count" in msg]
                    # ch_replies = channel_replies(reply_timestamps, msg)
...
def parse_replies(threads, users, channel_id):
    body = ""
    for thread in threads:
        body += parse_channel_history(thread, users, channel_id, check_thread=True)
        body += "\n"
...
if __name__ == "__main__":
    ...
    def save_replies(channel_hist, channel_id, channel_list, users):
        ...
        if a.json:
            ...
            data_replies = parse_replies(ch_replies, users, channel_id)
    ...
    def save_channel(channel_hist, channel_id, channel_list, users):
        if a.json:
            ...
            data_ch = parse_channel_history(channel_hist, users, channel_id)
            ch_name, ch_type = name_from_ch_id(channel_id, channel_list)
        ...
        save(data_ch, "channel_%s_%s" % (channel_id, ch_name))
